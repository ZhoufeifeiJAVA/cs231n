...
def batchnorm_backward(dout, cache):
    ...
    N, D = x.shape
    dx_1 = dout * gamma #dx_hat
    dx_2_b = np.sum(dx_1 * (x - u_b), axis = 0)
    dx_2_a = ((sigma_squared_b + eps) ** (-0.5)) * dx_1
    dx_3_b = -(1 / 2) * (sigma_squared_b + eps) ** (-1.5)  * dx_2_b #dsigma_squared_b
    dx_4_b = dx_3_b * 1
    dx_5_b = np.ones_like(x) / N * dx_4_b
    dx_6_b = 2 * (x - u_b) * dx_5_b
    dx_7_a = dx_6_b * 1 + dx_2_a * 1 #-du_b
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        mask = (np.random.rand(*x.shape) >= p) / (1 - p) #不理解此处为何还要除1-p ?
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            #max_x_masked.shape 为 (N,C)
            temp_binary_mask = (x_masked == (max_x_masked)[:, :, None, None])
            #(max_x_masked)[:, :, None, None].shape 为 N, C, 1, 1
