...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    h_int = np.matmul(x, Wx) + np.matmul(prev_h, Wh) + b
    next_h = np.tanh(h_int)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for turn in range(T):
        curr_x = x[:, turn, :]
        prev_h, cache_turn = rnn_step_forward(curr_x, prev_h, Wx, Wh, b)
        h[:, turn, :] = prev_h
        cache.append(cache_turn)
...
def rnn_backward(dh, cache):
    ...
    dpre_h = np.zeros_like(prev_h)
    for i in reversed(range(T)):
        dcurr_dh = dpre_h + dh[:, i, :]
        dx[:, i, :], dpre_h, dWx_turn, dWh_turn, db_turn = rnn_step_backward(dcurr_dh, cache[i])
        dWx += dWx_turn
        dWh += dWh_turn
        db += db_turn
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        pre_h, pre_c, cache_t = lstm_step_forward(x[:, t, :], pre_h, pre_c, Wx, Wh, b)
        h[:, t, :] = pre_h
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    _, Wx, Wh, _, _, _, _, _, _, _, _, _ = cache[0]
    N, T, H = dh.shape
    D, _ = Wx.shape
    ...
    dpre_h = np.zeros([N, H])
    dpre_c = np.zeros_like(dpre_h)

    for t in reversed(range(T)):
        dcurr_h = dpre_h + dh[:, t, :]
        dx[:, t, :], dpre_h, dpre_c, dWx_t, dWh_t, db_t = lstm_step_backward(dcurr_h, dpre_c, cache[t])
